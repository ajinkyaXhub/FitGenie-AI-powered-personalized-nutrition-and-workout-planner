...
@meal_bp.route("/profile", methods=["GET", "POST"])
@login_required
def profile():
    if request.method == "POST":
        current_user.age = request.form.get("age") or None
        current_user.gender = request.form.get("gender") or None
        current_user.height = request.form.get("height") or None
        current_user.weight = request.form.get("weight") or None
        current_user.goal = request.form.get("goal") or None
        current_user.diet = request.form.get("diet") or None
        current_user.activity = request.form.get("activity") or None
        current_user.cuisine = request.form.get("cuisine") or None

        db.session.commit()
        flash("Your profile has been updated.", "success")
        return redirect(url_for("meal.profile"))
...
@meal_bp.route("/settings", methods=["GET", "POST"])
@login_required
def settings():
    if request.method == "POST":
        ...
        if form_type == "account":
            ...
            if new_username and new_username != current_user.username:
                if User.query.filter_by(username=new_username).first():
                    ...
                    return redirect(url_for("meal.settings"))
        ...
        elif form_type == "reset_fitness_profile":
            ...
            db.session.commit()
            flash("Fitness profile data has been reset.", "success")
