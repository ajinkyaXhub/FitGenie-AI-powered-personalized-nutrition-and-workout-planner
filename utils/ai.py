...
def detect_mime_type(image_bytes):
    ...
    if image_bytes[4:12] in (b'ftypheic', b'ftypheix', b'ftyphevc', b'ftypmif1'):
        print("[AI] WARNING: HEIC/HEIF image detected — Gemini may not support this format directly.")
        return "image/heic"
    print(f"[AI] WARNING: Unrecognized image header {image_bytes[:12]!r} — defaulting to image/jpeg, results may be unreliable.")
    return "image/jpeg"
...
"""
    ...
    try:
        ...
        print(f"[AI] Sending {len(image_bytes)} bytes to model {Config.GEMINI_MODEL}...")

        # DO NOT use response_mime_type="application/json" — it breaks image analysis
        # low temperature keeps identification grounded in what's actually in the photo
        # instead of the model free-associating a plausible-sounding dish
